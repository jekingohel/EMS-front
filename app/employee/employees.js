...
export function Employees({ collection, ngn }) {
  const { toast } = useToast()
  const [editModal, setEditModal] = useState(false);
  const [selectItem, setSelectItem] = useState({});

  const handleContinue = (item) => {
    ngn.collection.delete.deleteEmployee(item)
  }
    ...
    <Table>
      ...
      <TableBody>
        {collection.map(employee => (
          <TableRow key={employee.id} className="w-auto">
            ...
            <TableCell className="text-end">
              ...
              <ConfirmDelete
                ...
                onContinue={() => handleContinue(employee)}
